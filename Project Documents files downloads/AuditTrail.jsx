...
if (result?.type === "longshort") {
  ...
  // Short starts the hole after Long closes (or hole 10 if Long never closes)
  const shortStartHole = result?.longDecidedOn ? result.longDecidedOn + 1 : 10;
  const shortUnits = shortStartHole <= 18
    ? holes
...
function OneVOneScorecard({ match, players, scores, course, handicapMode, result }) {
  ...
  const shortStartHole = longClosedOn ? longClosedOn + 1 : 10;
